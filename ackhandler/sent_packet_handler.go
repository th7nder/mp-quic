...
func (h *sentPacketHandler) detectLostPackets() {
	...
	for el := h.packetHistory.Front(); el != nil; el = el.Next() {
		...
		if timeSinceSent > delayUntilLost {
			utils.Debugf("LOST PACKET TIME: timeSinceSent: %s, sentTime: %d, now: %d, delayUntilLost: %s", timeSinceSent.String(), packet.SendTime.UnixNano(), now.UnixNano(), delayUntilLost.String())
			// Update statistics
			h.losses++
...
func (h *sentPacketHandler) onPacketAcked(packetElement *PacketElement) {
	h.dataGatherer.OnAckReceived(h.pathID, packetElement.Value.Frames)
